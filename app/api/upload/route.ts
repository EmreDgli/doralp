...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!allowedTypes.includes(file.type)) {
      return NextResponse.json({ error: 'Sadece JPEG, PNG, WebP ve GIF dosyaları kabul edilir' }, { status: 400 });
    }
    ...
    const fileName = `project-images/${timestamp}-${Math.random().toString(36).substring(2)}.${fileExtension}`;
